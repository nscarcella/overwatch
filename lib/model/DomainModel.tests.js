...
describe('DomainModel', () => {
	...
	describe('instances', () => {
		...
		describe('initialization', () => {
			...
			it('may receive an owner as second argument', () => {
				const owner = new domainClass()
				const instance = new domainClass({meh: true}, owner)

				expect(instance.owner).to.equal(owner)
			})
		})
	...
	describe('domainType', () => {
		...
		it('should add an "owner" reference to the children, targeting the parent', () => {
			const instance = new B({a: {name: 'foo'}, as: [{name: 'bar'}]})

			expect(instance.a.owner).to.equal(instance)
			expect(instance.as[0].owner).to.equal(instance)

			const a = instance.a
			a.owner = null

			expect(a.owner).to.be.null
		})
